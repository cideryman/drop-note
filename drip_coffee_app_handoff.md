...
| `v2.7.1` | 2026-08-02 | 테마 선택 UX | 선택 테마의 채움형 아이콘 버튼, 카드 내부 제목 정렬 및 깜빡임 없는 240ms 색상 전환 적용 | USER & Codex |
| `v2.8.0` | 2026-08-02 | THE NEO BREW | 테츠 카스야 공식 10푸어 프리셋과 누적값 기반 균등 반올림 계산 적용 | USER & Codex |
| `v2.9.0` | 2026-08-02 | 추출 시작 UX | 3·2·1 준비, 알림음 구분, 큰 글자, 재추출과 Skip 이중 확인 구현 | USER & Codex |
| `v3.0.0` | 2026-08-02 | 추출 기록 | 맛 평가·C40 제안·즐겨찾기·최근 사용과 기록 포함 백업 v2 구현 | USER & Codex |
...
- [x] **THE NEO BREW 10푸어 프리셋**: 20g·300g·96℃·C40 40–45클릭과 10회 다투입 타임라인 적용
- [x] **추출 시작·진행 UX**: 3·2·1 준비, 큰 글자, 재추출, 의미별 알림음과 Skip 이중 확인
- [x] **로컬 추출 기록**: 최근 100건, 빠른 맛 평가, C40 방향 제안, 즐겨찾기·최근 사용과 백업 v2
...
* **검증 결과**:
  ...
  * [Pass] 자동 회귀 테스트 7개 파일: 계산·타임라인·백업·기록·프리셋·테마·UX·PWA
...
---
*최종 업데이트: 2026-08-02 | 작성: USER, Antigravity AI & Codex*
...
- 코만단테 공식 일반 푸어오버 시작 범위는 18–35클릭이며, 앱의 세부 범위는 레시피 특성에 맞춘 시작점이지 절대값이 아니다.
- THE NEO BREW는 이 일반 범위보다 굵은 C40 40–45클릭을 테츠 카스야가 공식 레시피에서 직접 지정한 예외다.
...
### 7.21 THE NEO BREW · 10푸어 프리셋

- 테츠 카스야의 공식 영상 `THE NEO BREW (다투입 레시피)`를 원본 출처로 사용한다.
- 기준은 원두 20g, 물 300g, 1:15, 95–96℃, 코만단테 C40 표준 40–45클릭이며 앱의 명시적 온도 목표는 96℃로 둔다.
- 0:00에 30g을 붓고 30초간 뜸을 들인 뒤, 0:30부터 15초 간격으로 30g씩 추가해 2:30에 300g을 완성한다.
- 공식 안내의 `빠르고 균일하게 주입`을 실제 타이머에서 실행하기 쉽도록 각 30g 주입 구간을 5초로 표시한다. 이는 앱의 조작 가이드이며 공식이 별도로 지정한 고정 주입 시간은 아니다.
- 마지막 드로우다운 목표는 약 3:30이며 Hario V60 NEO를 권장 도구로 표시한다. 일반 V60도 사용할 수 있지만 소량의 물을 빠르고 균일하게 분산하는 것이 핵심이다.
- 원두량 변경 시 각 단계를 독립적으로 반올림하지 않고 누적 목표를 반올림한다. 예를 들어 15g에서는 `23·22·23·22…g`로 배분해 마지막 푸어에 오차가 몰리지 않으면서 총 225g을 정확히 유지한다.
- 공식 아이스 버전이 없으므로 아이스로 전환할 때는 기존 앱 정책인 추출수 1:10과 서버 얼음 1:5를 적용한다.
- 밝은 로스팅의 품질 좋은 원두를 우선 권장하며, 빠른 배수와 맛을 기준으로 분쇄도를 미세 조정한다.

### 7.22 추출 시작·진행 UX

- 타이머는 `preparing`, `running`, `paused`, `completed` 상태를 구분하며 준비 3초는 추출 경과 시간에 포함하지 않는다.
- 준비 화면에서 레시피·원두량·추출수·얼음·분쇄도·온도를 확인하고, 백그라운드 이동 시 시작을 취소한다.
- 실제 시작 시점부터 Wake Lock과 최근 사용을 활성화하고 단계·온도·동작·완료에 서로 다른 합성음을 사용한다.
- 완료 후 같은 조건으로 다시 준비할 수 있고 큰 글자 선택은 `DRIP_NOTE_LARGE_TIMER`에 저장한다.
- 다음 단계는 누적 저울 목표를 중심으로 표시하고 건너뛰기는 3초 안에 두 번 눌러야 실행한다.

### 7.23 추출 기록·즐겨찾기·백업 v2

- `DRIP_NOTE_BREW_HISTORY`에 최신 100건, `DRIP_NOTE_FAVORITES`에 즐겨찾기, `DRIP_NOTE_RECENT_RECIPES`에 최근 실제 시작 3건을 저장한다.
- 완료 즉시 기본 기록을 남기고 산미·쓴맛·농도, 원두명, C40 표준 클릭과 300자 메모를 선택적으로 보완한다.
- 명확한 맛 신호에만 C40 1–2클릭 조정 방향을 한 문장으로 제안하며 충돌하는 경우 같은 조건 재확인을 안내한다.
- 백업 버전 2는 커스텀 레시피·추출 기록·즐겨찾기·최근 사용을 포함하고 버전 1 백업도 계속 복원한다.
- 기록 저장 실패와 손상 데이터는 타이머 완료를 방해하지 않으며 계정·클라우드 동기화는 제공하지 않는다.
